...
class QuestionThree(object):
    def highest_temp(self, alist):
        ...
        for i in range(len(alist)):
            if max < alist[i]:
                max = alist[i] #replace with new value if greater
                index = i

        index = []
        for i in range(len(alist)): #calculates index of maxes
            if alist[i] == max:
                index.append(i)
    ...
    def lowest_temp(self, alist):
        ...
        min = 999
        for i in range(len(alist)):
            if min > alist[i]: #replace with new value if lesser
                min = alist[i]
                index = i
