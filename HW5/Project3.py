...
E.g.:
    ...
    def setPred(self, start=None, to=None, pred=None):
        ...
        try:
            ...
            self.vertList[start].connectedTo[to][1].append(pred)
            if len(self.getVertex(start).connectedTo[pred]) >= 2:
                self.setPred(start, to, self.getVertex(start).connectedTo[pred][1][0])
            else:
                pass
    ...
    def dijkstra(self, start):
        ...
        pq.heapInsert([(aGraph.getDistance(start.id, v.id), v) for v in aGraph if v != start])
            ...
            if currentVert:
                for nextVert_id in currentVert.getConnections():
                    ...
                    if newDist < aGraph.getDistance(start.id, nextVert_id):
                        ...
                        pq.decreaseKey(aGraph.getVertex(nextVert_id), newDist)  # repleace and heapify

    def betweenness_func(self):
        # TODO: fill this function
        """
        return bc_node, dictionary_of_betweenness
        ...
        for vertex in self.getVertices():
            preds = self.getVertex(vertex).getPreds()
            for pred in preds:
                if pred in dictionary_of_betweenness.keys():
                    dictionary_of_betweenness[pred] += 1
                else:
                    dictionary_of_betweenness[pred] = 1
